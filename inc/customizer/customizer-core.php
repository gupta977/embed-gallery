 ...
 * @return mixed
 */
function embed_default($theme_mod = '')
{

 $default = apply_filters('embed_default', array(
  'embed_read_more_label'       => esc_html_x('Read More', 'Read More Label', 'embed-gallery'),
  'embed_excerpt_length'        => 25,
  'embed_post_thumbnail_single' => false,
  'embed_theme_layout'          => 'wide',
  'embed_sidebar_position'      => 'right',
  'embed_copyright'             => sprintf('%1$s %2$s - <a href="%3$s">%4$s</a>', esc_html__('&copy; Copyright', 'embed-gallery'), esc_html(date_i18n(__('Y', 'embed-gallery'))), esc_attr(esc_url(home_url('/'))), esc_html(get_bloginfo('name'))),
  'embed_credit'                => true,
 ));
 ...
 * @return mixed
 */
function embed_mod($theme_mod = '')
{
 return get_theme_mod($theme_mod, embed_default($theme_mod));
}
...
if (class_exists('WP_Customize_Control')) {
 ...
 class embed_Button_Control extends WP_Customize_Control
 {
  /**
   * @access public
   * @var string
   */
  public $type = 'embed-gallery-button';
 ...
 */
function embed_sanitize_select($input, $setting)
{
 ...
 */
function embed_sanitize_checkbox($checked)
{
